      ...
      <div className="relative px-11 py-8 max-[860px]:px-[18px]">
        ...
        <div className="relative z-10 mx-auto max-w-[1220px]">
          ...
          <div className="grid grid-cols-[minmax(0,1fr)_480px] gap-7 max-[1100px]:grid-cols-1">
            <section className="comic-panel rounded p-7">
            ...
            <aside className="comic-panel overflow-hidden rounded">
              <div className="relative h-[560px] max-[1100px]:h-[460px]">
                ...
                <div className="absolute inset-0 bg-[linear-gradient(180deg,transparent_30%,rgba(0,0,0,.9)_100%)]" />
                <div className="comic-halftone absolute inset-0 opacity-30" />
                <div className="absolute left-4 top-4 inline-flex rotate-[-1.5deg] items-center gap-2 rounded bg-[#f5a623] px-3 py-[7px] text-[11px] font-black uppercase tracking-[1.8px] text-black shadow-[4px_4px_0_#000]">
                  <ImageIcon size={14} /> Panel {Math.max(activeIndex, 0) + 1}
                </div>
            ...
            {scene.lines.map((l, i) => {
              ...
              return (
                <motion.div key={i} initial={{ opacity: 0, x: -14 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 + i * 0.04, duration: 0.35 }}
                  className="comic-strip-panel grid grid-cols-[92px_130px_minmax(0,1fr)] gap-4 rounded p-4 transition max-[760px]:grid-cols-[76px_minmax(0,1fr)]"
                  style={{ borderColor: active ? "rgba(245,166,35,.9)" : "rgba(255,255,255,.16)", boxShadow: active ? "6px 6px 0 #000, 0 0 0 2px rgba(245,166,35,.55)" : undefined }}>
                  <div className="relative h-[92px] overflow-hidden rounded bg-white/7 ring-2 ring-black max-[760px]:h-[76px]">
                    {thumbUrl ? (
                      // eslint-disable-next-line @next/next/no-img-element
                    ...
                    {active && <div className="absolute inset-0 ring-2 ring-inset ring-[#f5a623]" />}
                  </div>
                  ...
                  <div className="max-[760px]:col-span-2">
                    <div className="speech-bubble text-[15px] font-semibold leading-7 text-white/88">{l.text}</div>
...
function ActiveDialogue({ line, mood }: { line: Scene["lines"][number] | undefined; mood: string }) {
  return (
    <div className="mt-7 grid grid-cols-[132px_minmax(0,1fr)] overflow-hidden rounded bg-black/32 ring-2 ring-black max-[680px]:grid-cols-1">
      <div className="border-r-2 border-black bg-[#e50914] p-5 max-[680px]:border-b-2 max-[680px]:border-r-0">
        <div className="text-[11px] font-black uppercase tracking-[2px] text-white/72">Now Speaking</div>
        <div className="mt-3 text-[18px] font-black leading-tight text-white">{line?.speaker || "Narrator"}</div>
        <div className="mt-3 inline-flex rounded bg-black/24 px-3 py-1 text-[10.5px] font-black uppercase tracking-[1.5px] text-white ring-1 ring-white/20">
          {line?.expression || line?.emotion || mood}
        </div>
      </div>
      <div className="speech-bubble m-5 p-5">
